...
class SniIntegrationTests {
	...
	private static final Map<String, String> SERVER_START_MESSAGE = Map.ofEntries(Map.entry("netty", "Netty started"),
	...
	void home(String webStack, String server) {
		try (ApplicationContainer serverContainer = new ServerApplicationContainer(webStack, server)) {
			...
			assertThat(serverLogs).contains(SERVER_START_MESSAGE.get(server));
